...
// Anvil default accounts for local testing
...
export function getLocalWalletClient(accountIndex: number) {
  const acc = ANVIL_ACCOUNTS[accountIndex]
  if (!acc) throw new Error('Invalid account index')
  const account = privateKeyToAccount(acc.key as `0x${string}`)
